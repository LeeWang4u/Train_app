...
public class TrainSearchResultActivity extends AppCompatActivity {
    ...
    private void changeDate(int offsetDays, TripRequest tripRequest) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // Tạo TripRequest mới với ngày cập nhật
            TripRequest updatedTripRequest = new TripRequest(tripRequest.getDepartureStation(),
                    tripRequest.getArrivalStation(), currentTripDate.format(ISO_DATE_FORMATTER));
            updatedTripRequest.setDepartureStation(originalTripRequest.getDepartureStation());
            updatedTripRequest.setArrivalStation(originalTripRequest.getArrivalStation());
            updatedTripRequest.setTripDate(currentTripDate.format(DateTimeFormatter.ISO_LOCAL_DATE));

            // Xóa danh sách cũ và lấy danh sách chuyến tàu mới
            trips.clear();
            adapter.notifyDataSetChanged();
            fetchTrips(updatedTripRequest);
        }
    }
